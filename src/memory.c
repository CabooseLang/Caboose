...
static void grayArray(ValueArray *array) {
    for (int i = 0; i < array->count; i++) grayValue(array->values[i]);
}
...
static void blackenObject(Obj *object) {
    ...
    switch (object->type) {
        ...
        case OBJ_CLOSURE: {
            ...
            for (int i = 0; i < closure->upvalueCount; i++) grayObject((Obj *) closure->upvalues[i]);
            break;
        }
...
void collectGarbage() {
    ...
    for (Value *slot = vm.stack; slot < vm.stackTop; slot++) grayValue(*slot);

    for (int i = 0; i < vm.frameCount; i++) grayObject((Obj *) vm.frames[i].closure);

    // Mark the open upvalues.
    for (ObjUpvalue *upvalue = vm.openUpvalues; upvalue != NULL; upvalue = upvalue->next) grayObject((Obj *) upvalue);
