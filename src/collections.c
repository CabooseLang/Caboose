...
static bool insertListItem(int argCount) {
    ...
    for (int i = list->values.count - 1; i > index; --i) list->values.values[i] = list->values.values[i - 1];
...
static bool popListItem(int argCount) {
    ...
    if (argCount == 1) {
        ...
        for (int i = index; i < list->values.count - 1; ++i) list->values.values[i] = list->values.values[i + 1];
    }
    list->values.count--;
    push(last);
...
static bool containsListItem(int argCount) {
    ...
    for (int i = 0; i < list->values.capacity; ++i) {
        if (!list->values.values[i]) continue;
...
ObjList* copyList(ObjList *oldList, bool shallow) {
    ...
    for (int i = 0; i < oldList->values.count; ++i) {
        ...
        if (!shallow) {
            if (IS_DICT(val)) val = OBJ_VAL(copyDict(AS_DICT(val), false));
            else if (IS_LIST(val)) val = OBJ_VAL(copyList(AS_LIST(val), false));
        }
...
bool listMethods(char *method, int argCount) {
    if (strcmp(method, "push") == 0) return pushListItem(argCount);
    else if (strcmp(method, "insert") == 0) return insertListItem(argCount);
    else if (strcmp(method, "pop") == 0) return popListItem(argCount);
    else if (strcmp(method, "contains") == 0) return containsListItem(argCount);
    else if (strcmp(method, "copy") == 0) return copyListShallow(argCount);
    else if (strcmp(method, "deepCopy") == 0) return copyListDeep(argCount);
...
static bool getDictItem(int argCount) {
    ...
    if (ret == NIL_VAL) push(defaultValue);
    else push(ret);
...
static bool removeDictItem(int argCount) {
    ...
    if (dict->items[index]) {
        ...
        if (dict->capacity != 8 && dict->count * 100 / dict->capacity <= 35) resizeDict(dict, false);
...
ObjDict *copyDict(ObjDict *oldDict, bool shallow) {
    ...
    for (int i = 0; i < oldDict->capacity; ++i) {
        if (oldDict->items[i] == NULL) continue;
...
bool dictMethods(char *method, int argCount) {
    if (strcmp(method, "get") == 0) return getDictItem(argCount);
    else if (strcmp(method, "remove") == 0) return removeDictItem(argCount);
    else if (strcmp(method, "exists") == 0) return dictItemExists(argCount);
    else if (strcmp(method, "copy") == 0) return copyDictShallow(argCount);
    else if (strcmp(method, "deepCopy") == 0) return copyDictDeep(argCount);
