...
static int resolveLocal(Compiler *compiler, Token *name, bool inFunction) {
    ...
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            if (!inFunction && local->depth == -1) error("Cannot read local variable in its own initializer.");
            return i;
        }
    }
...
static void declareVariable() {
    ...
    for (int i = current->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) error("Variable with this name already declared in this scope.");
    }
...
static uint8_t parseVariable(const char *errorMessage) {
    ...
    if (current->scopeDepth == 0) return identifierConstant(&parser.previous);
...
static uint8_t argumentList() {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) do {
        ...
        if (argCount > 32) error("Cannot have more than 32 arguments.");
...
static void subscript(bool canAssign) {
    ...
    TokenType type;
    if (parser.previous.type == TOKEN_NUMBER) type = TOKEN_NUMBER;
    else type = TOKEN_STRING;
    ...
    if (type == TOKEN_NUMBER) {
        ...
        } else emitByte(OP_SUBSCRIPT);
    } else {
        // Dict subscript
        if (match(TOKEN_EQUAL)) {
            expression();
            emitByte(OP_SUBSCRIPT_DICT_ASSIGN);
        } else emitByte(OP_SUBSCRIPT_DICT);
    }
}
...
static void super_(bool canAssign) {
    if (currentClass == NULL) error("Cannot use 'super' outside of a class.");
    else if (!currentClass->hasSuperclass) error("Cannot use 'super' in a class with no superclass.");
...
static void this_(bool canAssign) {
    ...
    else if (staticMethod) error("Cannot use 'this' inside a static method.");
    else variable(false);
}

static void static_(bool canAssign) {
    if (currentClass == NULL) error("Cannot use 'static' outside of a class.");
}
...
static void prefix(bool canAssign) {
    ...
    if (instance) emitBytes(OP_SET_PROPERTY, arg);
    else {
        uint8_t setOp;
        int arg = resolveLocal(current, &cur, false);

        if (arg != -1) setOp = OP_SET_LOCAL;
        else if ((arg = resolveUpvalue(current, &cur)) != -1) setOp = OP_SET_UPVALUE;
        else {
...
static void function(FunctionType type) {
    ...
    if (!check(TOKEN_RIGHT_PAREN)) {
        do {
            ...
            if (current->function->arity > 32) error("Cannot have more than 32 parameters.");
        } while (match(TOKEN_COMMA));
    }
    ...
    if (type == TYPE_STATIC) staticMethod = false;
...
static void classDeclaration() {
    ...
    } else emitBytes(OP_CLASS, nameConstant);
...
static void continueStatement() {
    if (innermostLoopStart == -1) error("Cannot use 'continue' outside of a loop.");

    consume(TOKEN_SEMICOLON, "Expect ';' after 'continue'.");

    // Discard any locals created inside the loop.
    for (int i = current->localCount - 1; i >= 0 && current->locals[i].depth > innermostLoopScopeDepth; i--) emitByte(OP_POP);
...
static void returnStatement() {
    if (current->type == TYPE_TOP_LEVEL) error("Cannot return from top-level code.");
...
static void whileStatement() {
    ...
    if (check(TOKEN_LEFT_BRACE)) emitByte(OP_TRUE);
    else {
...
static void statement() {
    if (match(TOKEN_FOR)) forStatement();
        ...
        if (check(TOKEN_STRING)) {
            for (int i = 0; i < parser.current.length - parser.previous.length + 1; ++i) backTrack();
