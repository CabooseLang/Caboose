...
static bool readLineFile(int argCount) {
    ...
    ObjFile *file = AS_FILE(pop());
    if (fgets(line, 4096, file->file) != NULL) push(OBJ_VAL(copyString(line, strlen(line))));
    else push(OBJ_VAL(copyString("", 0)));
...
bool fileMethods(char *method, int argCount) {
    ...
    else if (strcmp(method, "readLine") == 0) return readLineFile(argCount);
    else if (strcmp(method, "seek") == 0) return seekFile(argCount);
