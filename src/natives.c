...
static Value typeNative(int argCount, Value *args) {
    ...
    else if (IS_NIL(args[0])) return OBJ_VAL(copyString("nil", 3));
    else if (IS_NUMBER(args[0])) return OBJ_VAL(copyString("number", 6));
    else if (IS_OBJ(args[0])) switch (OBJ_TYPE(args[0])) {
        case OBJ_CLASS:
...
static Value sumNative(int argCount, Value *args) {
    ...
    if (argCount == 0) return NUMBER_VAL(0);
