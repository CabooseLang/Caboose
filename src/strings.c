...
static bool replaceString(int argCount) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (i == 0) snprintf(newStr, length, "%s", tmp1);
        else strncat(newStr, tmp1, strlen(tmp1));
...
static bool lowerString(int argCount) {
    ...
    for(int i = 0; string->chars[i]; i++) temp[i] = tolower(string->chars[i]);
...
static bool upperString(int argCount) {
    ...
    for(int i = 0; string->chars[i]; i++)temp[i] = toupper(string->chars[i]);
...
static bool rightStripString(int argCount) {
    ...
    for (length = string->length - 1; length > 0; --length) if (!isspace(string->chars[length])) break;
...
static bool formatString(int argCount) {
    ...
    for (int j = argCount - 2; j >= 0; --j) {
        Value value = pop();
        if (!IS_STRING(value)) replace_strings[j] = valueToString(value);
        else {
    ...
    if (count != argCount - 1) {
        ...
        for (int i = 0; i < argCount - 1; ++i) free(replace_strings[i]);
    ...
    for (int i = 0; i < argCount - 1; ++i) {
        ...
        if (i == 0) snprintf(newStr, fullLength, "%s", tmp1);
        else strncat(newStr, tmp1, strlen(tmp1));
...
bool stringMethods(char *method, int argCount) {
    if (strcmp(method, "split") == 0) return splitString(argCount);
    else if (strcmp(method, "contains") == 0) return containsString(argCount);
    else if (strcmp(method, "find") == 0) return findString(argCount);
    else if (strcmp(method, "replace") == 0) return replaceString(argCount);
    else if (strcmp(method, "lower") == 0) return lowerString(argCount);
    else if (strcmp(method, "upper") == 0) return upperString(argCount);
    else if (strcmp(method, "startsWith") == 0) return startsWithString(argCount);
    else if (strcmp(method, "endsWith") == 0) return endsWithString(argCount);
    else if (strcmp(method, "leftStrip") == 0) return leftStripString(argCount);
    else if (strcmp(method, "rightStrip") == 0) return rightStripString(argCount);
    else if (strcmp(method, "strip") == 0) return stripString(argCount);
    else if (strcmp(method, "format") == 0) return formatString(argCount);
