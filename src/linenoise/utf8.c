...
static int cmp_range(const void *key, const void *cm) {
    ...
    int ch = *(int *)key;
    if (ch < range->lower) return -1;
    if (ch >= range->upper) return 1;
    return 0;
}

static int utf8_in_range(const struct utf8range *range, int num, int ch) {
    const struct utf8range *r = bsearch(&ch, range, num, sizeof(*range), cmp_range);
...
int utf8_width(int ch) {
    ...
    if (utf8_in_range(unicode_range_combining, ARRAYSIZE(unicode_range_combining), ch)) return 0;
    if (utf8_in_range(unicode_range_wide, ARRAYSIZE(unicode_range_wide), ch)) return 2;
    return 1;
}
#endif