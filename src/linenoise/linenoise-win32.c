...
void linenoiseClearScreen(void) {
	...
	if (getWindowSize(&current) == 0) {
		...
		FillConsoleOutputAttribute(
			current.outh, FOREGROUND_RED | FOREGROUND_BLUE | FOREGROUND_GREEN,
			current.cols * current.rows, topleft, &n);
...
#define UNI_MAX_BMP 0x0000FFFF
	...
	if (ch > UNI_MAX_BMP) {
		 ...
		 */
		ch -= halfBase;
		current->ubuf[current->ubuflen++] =
			(WORD)((ch >> halfShift) + UNI_SUR_HIGH_START);
		current->ubuf[current->ubuflen++] =
			(WORD)((ch & halfMask) + UNI_SUR_LOW_START);
	} else {
...
static void eraseEol(struct current *current) {
	...
	FillConsoleOutputCharacter(current->outh, ' ', current->cols - current->x,
							   pos, &n);
}
	...
	while (1) {
		...
		if (irec.EventType == KEY_EVENT) {
			...
			if (k->bKeyDown || k->wVirtualKeyCode == VK_MENU) {
				...
				/* Note that control characters are already translated in
				   AsciiChar */
