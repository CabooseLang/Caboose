...
#define CASE_CODE(name) case OP_##name
	...
	INTERPRET_LOOP {
		...
		DISPATCH();
		CASE_CODE(NEGATE) : if (!IS_NUMBER(peek(0))) {
			runtimeError("Operand must be a number.");
			return INTERPRET_RUNTIME_ERROR;
		}
		...
		CASE_CODE(CALL_0)
			...
			: CASE_CODE(CALL_27)
			: CASE_CODE(CALL_28)
			: CASE_CODE(CALL_29) : CASE_CODE(CALL_30) : CASE_CODE(CALL_31) : {
		...
		CASE_CODE(INVOKE_0)
			...
			: CASE_CODE(INVOKE_28)
			: CASE_CODE(INVOKE_29)
			: CASE_CODE(INVOKE_30) : CASE_CODE(INVOKE_31) : {
		...
		CASE_CODE(SUPER_0)
			...
			: CASE_CODE(SUPER_28)
			: CASE_CODE(SUPER_29)
			: CASE_CODE(SUPER_30) : CASE_CODE(SUPER_31) : {
			ObjString *method = READ_STRING();
		...
		CASE_CODE(CLASS) : createClass(READ_STRING(), NULL);
		DISPATCH();
		...
		CASE_CODE(METHOD) : defineMethod(READ_STRING());
		DISPATCH();
		CASE_CODE(OPEN_FILE) : {
			Value openType = pop();
			Value fileName = pop();
